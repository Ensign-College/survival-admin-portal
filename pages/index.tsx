        ...
        <div className="container p-4 mx-auto">
            ...
            <div className="space-y-4">
                {cards.map((card, index) => (
                    <div key={index} className="flex items-center p-4 border rounded shadow-lg justify-between"
                        ...
                        <div className="flex items-center">
                            {card.image_logo === "https://example.com/logo.png" ? (
                                ...
                                <img src={card.image_logo} alt={card.title}
                                     className="object-cover w-16 h-16 mr-4 max-w-full max-h-full"/>
                            )}
                            <h2 className="text-xl flex-shrink-0">{card.title}</h2>
                        </div>
                        <div className="button-container">
                            <button onClick={() => handleEdit(card.id)}
                                    className="px-2 py-1 text-slate-400 bg-transparent hover:bg-green-500 rounded hover:text-white">
                                Edit
                            </button>
                            <button onClick={() => handleDelete(card.id)}
                                    className="px-2 py-1 text-white bg-transparent hover:bg-red-500 rounded hover:text-white">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                     stroke="currentColor" className="h-6 w-6 fill-red-600">
